...
Added a small mock evaluation file at:

```text
baselines/data/medqa/rcc_mock_eval.jsonl
```

Generated method-level prediction files in:
...
The suite now exercises the paper's central ablation idea: removing retrieval removes evidence support; removing safety removes escalation behavior; removing guideline verification or safety breaks full audit-trace completeness. Accuracy is intentionally not yet differentiated because the mock model is a deterministic stand-in and the evaluation set is tiny.

The current result supports the engineering claim that the framework can emit comparable schemas across baselines and module ablations. It does not yet support a scientific claim about clinical QA performance.
...
```bash
python3 -m src.rcc_agents.run_ablation_suite --input baselines/data/medqa/rcc_mock_eval.jsonl
python3 -m compileall src/rcc_agents
```
